...
import { CheckCircle2, AlertCircle, TrendingUp, Headphones, Target, Search, Ear, Brain } from 'lucide-react'
...
export default function ListeningResults({ scoreData }: ListeningResultsProps) {
    ...
    const getProgressColor = (score: number, max: number = 1) => {
        const ratio = score / max
    ...
    const criteria = [
        { label: 'Accuracy', data: scoreData.accuracy, icon: Target, max: 1 },
        { label: 'Comprehension', data: scoreData.comprehension, icon: Brain, max: 1 },
        { label: 'Retention', data: scoreData.retention, icon: Ear, max: 1 },
    ].filter(item => item.data !== undefined)

    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            {/* Overall Score Large Card */}
            <div className="relative overflow-hidden rounded-[40px] bg-gradient-to-br from-orange-900 to-amber-950 p-1">
                <div className="bg-background rounded-[38px] p-8 md:p-12 relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-12 relative z-10">
                        <div className="text-center md:text-left space-y-4">
                            <Badge className="bg-orange-500/10 text-orange-500 border-none font-black uppercase tracking-widest text-[10px] px-4 py-1">
                                Listening Proficiency
                            </Badge>
                            <h2 className="text-4xl md:text-5xl font-black tracking-tight text-foreground">Listening <br/>Analysis</h2>
                            <p className="text-muted-foreground font-medium text-lg max-w-md">Detailed breakdown of your auditory comprehension and academic accuracy.</p>
                        </div>
                        ...
                        <div className="relative flex items-center justify-center">
                            <div className="size-48 md:size-56 rounded-full border-8 border-muted flex flex-col items-center justify-center relative">
                                <span className="text-6xl md:text-7xl font-black tracking-tighter text-orange-600">
                    ...
                    <Card key={item.label} className="border-border/40 rounded-[32px] overflow-hidden bg-card/30 backdrop-blur-sm shadow-none">
                        <CardContent className="p-6 space-y-4">
                            <div className="flex items-center justify-between">
                                <div className="size-10 rounded-xl bg-orange-500/5 flex items-center justify-center">
                                    <item.icon className="size-5 text-orange-500" />
                                </div>
                                <span className={cn("text-xl font-black", getScoreColor(item.data!.score))}>
                            ...
                            <div>
                                ...
                                <p className="text-[10px] font-bold leading-relaxed text-muted-foreground/80 line-clamp-3 italic">
                                    &ldquo;{item.data!.feedback}&rdquo;
                                </p>
                            </div>
                        </CardContent>
                    </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="p-8 rounded-[40px] bg-emerald-500/5 border border-emerald-500/10 space-y-6">
                    <div className="flex items-center gap-3 text-emerald-600">
                        ...
                        <h4 className="font-black text-sm uppercase tracking-widest">Auditory Strengths</h4>
                    </div>
                ...
                <div className="p-8 rounded-[40px] bg-rose-500/5 border border-rose-500/10 space-y-6">
                    <div className="flex items-center gap-3 text-rose-600">
                        ...
                        <h4 className="font-black text-sm uppercase tracking-widest">Critical Gaps</h4>
                    </div>
                    <ul className="space-y-3">
